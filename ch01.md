...
A small program to test your knowledge
======================================

To start things off we'll make a tiny program that checks your programming knowledge (that you don't have
yet!)

You can use the filename `pytest.py` to store this code. To run it, you will need to type `python3 pytest.py` in your terminal.

The program will use the following commands:

- creating a list of text strings
- getting input from the user
- checking if one thing is equal to another

I will first define questions and answers in a list, as shown below:
...
Note that a list is created with square brackets and each piece of text is surrounded by single quotes. Items
within a list are separated by commas.

We'll go into more detail about each of these topics later in the chapter, but for now let's look at the code
that asks the questions. It also checks if the answer is right after the user types in his or her response:
...
- `for` command goes over our questions in a list, one by one.
- `=` operator assigns to a variable.
- `==` operator checks if something is equal to another thing.
- `if` command does an action if two things are equal.
- `print()` command displays given text to the screen
